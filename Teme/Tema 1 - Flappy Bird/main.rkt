...
(define-struct bird-t (x y v-y) #:transparent)
(define-struct pipe-t (x gap-y) #:transparent)
(define-struct state-t (bird-t score pipes) #:transparent)

(define (get-initial-state)
  [define initial-bird (bird-t bird-x bird-initial-y 0)]
  [define initial-pipes (pipe-t scene-width (+ added-number (random random-threshold)))]
  [define initial-state (state-t initial-bird 0 (list initial-pipes))]
  initial-state)
...
(define (next-state-bird bird gravity)
  (match-let* ([(bird-t _ y v-y) bird])
    (struct-copy bird-t bird [y (+ y v-y)]
                 [v-y (+ v-y gravity)])))
...
(define (next-state-bird-onspace bird momentum)
  (match-let* ([(bird-t _ _ v-y) bird])
    (struct-copy bird-t bird [v-y (* -1 momentum)])))
...
(define (change current-state pressed-key)
  (if (key=? pressed-key " ") (struct-copy state-t current-state [bird-t (next-state-bird-onspace (get-bird current-state) initial-momentum)])
      current-state))
...
(define (move-pipes pipes scroll-speed)
  (let ((f (λ (pipe) (struct-copy pipe-t pipe [x (- (get-pipe-x pipe) scroll-speed)]))))
    (map f pipes)))
...
(define (clean-pipes pipes)
  (let ((f (λ (pipe) (>= (get-pipe-x pipe) (* -1 pipe-width)))))
    (filter f pipes)))
...
(define (highest-number L)
  (let ((f (λ (a b) (if (< a b) b a))))
    (foldl f (car L) (cdr L))))

(define (add-more-pipes pipes)
  (if (>= (length pipes) no-pipes) pipes
      (cons (pipe-t (+ (+ pipe-width pipe-gap) (highest-number (map get-pipe-x pipes))) (+ added-number (random random-threshold))) pipes)))
...
(define (bird-coordinates bird)
  (match-let ([(bird-t x y _) bird])
    (list [make-posn x y] [make-posn (+ x bird-width) (+ y bird-height)])))

(define (upper-pipe-coordinates pipes)
  (list [make-posn (get-pipe-x (car pipes)) 0] [make-posn (+ (get-pipe-x (car pipes)) pipe-width) (pipe-t-gap-y (car pipes))]))

(define (lower-pipe-coordinates pipes)
  (list [make-posn (get-pipe-x (car pipes)) (+ (pipe-t-gap-y (car pipes)) pipe-self-gap)]
        [make-posn (+ (get-pipe-x (car pipes)) pipe-width) ground-y]))

(define (check-pipe-collisions bird pipes)
  (cond  [(null? pipes) #f]
         [(or (apply check-collision-rectangles (append (bird-coordinates bird) (upper-pipe-coordinates pipes)))                   
              (apply check-collision-rectangles (append (bird-coordinates bird) (lower-pipe-coordinates pipes))))
          #t]
...
(define (next-state state)
  (state-t (next-state-bird (get-bird state) initial-gravity) (+ (get-score state) 0.1) (next-state-pipes (get-pipes state) initial-scroll-speed)))
...
(define (frame-images state)
  (match-let ([(state-t _ score _) state])
    (list bird-image ground-image (score-to-image score))))

(define (frame-images-coordinates state)
  (match-let ([(state-t bird-t score pipes) state])
    (list [make-posn (+ bird-x (quotient bird-width 2)) (+ (get-bird-y bird-t) (quotient bird-height 2))]
          [make-posn (quotient scene-width 2) (- scene-height (quotient ground-height 2))]
          [make-posn text-x text-y])))

(define (draw-frame state)
  (match-let ([(state-t bird-t score pipes) state])
    (place-images (frame-images state) (frame-images-coordinates state) (place-pipes pipes initial-scene))))

; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.

(define (pipes-coordinates pipes)
  (list [make-posn (+ (get-pipe-x (car pipes)) (quotient pipe-width 2)) (- (pipe-t-gap-y (car pipes)) (quotient pipe-height 2))]
        [make-posn (+ (get-pipe-x (car pipes)) (quotient pipe-width 2)) (+ (pipe-t-gap-y (car pipes)) (+ pipe-self-gap (quotient pipe-height 2)))]))

(define (place-pipes pipes scene)
  (if (null? pipes)
      scene
      (place-pipes (cdr pipes) (place-images (list pipe-image pipe-image) (pipes-coordinates pipes) scene))))
...
; move-abilities, clean-abilities, etc..
