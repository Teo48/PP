...
unzip2  :: [(a, b)] -> ([a], [b])
unzip2 [] = ([], [])
unzip2 xs =  (fst (head xs) : l1, snd (head xs) : l2) where (l1, l2) = unzip2 (tail xs)
...
divizori d = 1 : [i | i <- [2 .. (int_sqrt d)], d `mod` i == 0] ++ [d]

is_prime d = divizori d == [1, d]

primes :: Int -> [Int]
primes n = [i | i <- [2 .. n], is_prime i]
...
group2 :: Eq a => [a] -> [[a]]
group2 [] = []
group2 (x : xs) = (x : ys) : group2 zs where (ys, zs) = span (== x) xs
...
dup :: String -> String
duplicate x = x ++ " " ++ x
dup = unwords . map (duplicate). words
