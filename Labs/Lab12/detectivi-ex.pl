...
suspect(Nume:Marca:Arma) :- inarmat(Nume, Arma), conduce(Nume, Marca).
...
au_marca(Marca, ListaNume) :- findall(X, conduce(X, Marca), ListaNume).
...
arme_bmw(ListaArme) :- findall(Y, (inarmat(X, Y), conduce(X, bmw)), ListaArme).
...
arme_marca(Marca, ListaArme) :- bagof(Y, X^(inarmat(X, Y), conduce(X, Marca)), ListaArme).
...
marci_arma_unica(ListaMarci) :- findall(Marca, (arme_marca(Marca, ListaArme), length(ListaArme, Size), Size == 1), ListaMarci).
