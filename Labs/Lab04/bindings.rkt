...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let*  [(segment (get-line-segment))
          (starting-point (get-start-point segment))
          (ending-point (get-end-point segment))
          (x-a (car starting-point))
          (y-a (cdr starting-point))
          (x-b (car ending-point))
          (y-b (cdr ending-point))]
...
(define (distance p1 p2)
  (let* [(segment (cons p1 p2))
         (get-segment (lambda () segment))
         (start-point (lambda (segment) p1))
         (end-point (lambda (segment) p2))  ]
    (compute-length get-segment start-point end-point)) )
...
(define (compute-perimeter points)
  (let perimeter ((current-points points))
    (if (null? (cdr current-points))
        (distance (car points) (car current-points))
        (+ (distance (car current-points) (cadr current-points))
           (perimeter (cdr current-points))))))
