...
normalizeV (V vx vy vz) = let norm = lengthV (V vx vy vz) in V (vx / norm) (vy / norm) (vz / norm)

dotV :: Vector -> Vector -> Double
dotV (V x1 y1 z1) (V x2 y2 z2) = x1 * x2 + y1 * y2 + z1 * z2

crossV :: Vector -> Vector -> Vector
crossV (V x1 y1 z1) (V x2 y2 z2) = V (y1 * z2 - y2 * z1) (z1 * x2 - z2 * x1) (x1 * y2 - y1 * x2)
...
orthogonalV vectors = case sigma == 0 of
  True -> True
  _ -> False
  where sigma = sum $ map (dotV (head vectors)) (tail vectors)
...
{-
 2. (4p) Liste imbricate
  ...
  (1p) flatten
-}
data SList a = UndefinedSList | Cons a (SList a) | List [SList a] | Merge (SList a) (SList a) deriving Show

emptySList :: SList a
emptySList = UndefinedSList

consElem :: a -> SList a -> SList a
consElem x xs = Cons x xs

consList :: SList a -> SList a -> SList a
consList UndefinedSList UndefinedSList = UndefinedSList
consList xs ys = Merge xs ys

headSList :: SList a -> SList a
headSList UndefinedSList = UndefinedSList
headSList (Cons a xs) = Cons a UndefinedSList
headSList (Merge xs ys) = xs
headSList (List xs) = head xs

tailSList :: SList a -> SList a
tailSList UndefinedSList = UndefinedSList
tailSList (Cons a xs) = xs
tailSList (List xs) = List (tail xs)

deepEqual :: Eq a => SList a -> SList a -> Bool
deepEqual UndefinedSList UndefinedSList = True
deepEqual (Cons a xs) (Cons b ys) = (a == b) && (deepEqual xs ys)
deepEqual (List a) (List b) = case length a /= length b of
  False -> False
  _ -> and $ zipWith deepEqual a b
deepEqual (Merge xs ys) (Merge zs ws) = (deepEqual xs zs) && (deepEqual ys ws)


flatten :: SList a -> [a]
flatten UndefinedSList = []
flatten (Cons a xs) = (a : flatten xs)
flatten (Merge xs ys) = (flatten xs) ++ (flatten ys)
...
insertElem (BSTNode n left right) x = case x < n of
  True -> BSTNode n (insertElem left x) right
  _ -> BSTNode n left (insertElem right x)

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil _ = Nothing
findElem (BSTNode n left right) x
	| x == n = Just x
	| x < n = findElem left x
	| otherwise = findElem right x

subTree :: (Ord a, Eq a) => BST a -> a -> a -> Maybe (BST a)
subTree (BSTNode root leftNode rightNode) x y
  | findElem (BSTNode root leftNode rightNode) x == Nothing || findElem (BSTNode root leftNode rightNode) y == Nothing = Nothing
  | x <= root && y <= root = subTree leftNode x y
  | x > root && y > root   = subTree rightNode x y
  | otherwise = Just (BSTNode root leftNode rightNode)

inorder :: BST a -> [a]
inorder BSTNil = []
inorder (BSTNode a left right) = inorder left ++ [a] ++ inorder right
