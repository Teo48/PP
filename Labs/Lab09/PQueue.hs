...
class (Ord a) => PQueue pq a where
    ...
    toList q = case isEmpty q of 
      True -> []
      _ -> (maybeToList $ top q) ++ (toList $ pop q)

    size :: pq a -> Int;
	  size q = case isEmpty q of
      True -> 0
      _ -> 1 + (size $ pop q)
...
instance (Ord a) => PQueue ListPQ a where
    ...
    insert x (LPQ []) = (LPQ [x])
    insert x (LPQ l) = let f = (\x y -> compare (fst y) (fst x)) in LPQ (sortBy f $ x : l)
...
merge (Empty x) (Node y root l r) = (Node y root l r)

merge (Node x root l r) (Empty y) = (Node x root l r)

merge (Node x1 root1 l1 r1)  (Node x2 root2 l2 r2 )
  | fst root1 > fst root2 = merge (Node x2 root2 l2 r2)(Node x1 root1 l1 r1)
  | x1 >= x2 = (Node (x2 + 1) root1 l1 (merge r1 (Node x2 root2 l2 r2)))
  | otherwise = (Node (x1 + 1) root1 (merge r1 (Node x2 root2 l2 r2)) l1)

inorder :: LeftistPQ a -> [(Prio, a)]
inorder (Node x root (Empty l) (Empty r)) = [root]

inorder (Node x root l (Empty r)) = (inorder l) ++ [root]

inorder (Node x root (Empty l) r) = [root] ++ (inorder r)

inorder (Node x root l r) = (inorder l) ++ [root] ++ (inorder r)
...
instance MyFoldable ListPQ where
    foldr' f acc (LPQ q) = let func = (\ elem -> (f (snd elem))) in foldr func acc q
