...
arithmetic initial ratio = iterate (\x -> x + ratio) initial
geometric :: Num a => a -> a -> [a]
geometric initial ratio = iterate (\x -> x * ratio) initial
{-
  3. (2p)
...
randoms :: Int -> [Word8]
randoms seed = getNextPseudo $ next $ mkStdGen seed where getNextPseudo (x, gen) = (fromIntegral x) : (getNextPseudo $ next gen)
{-
    4. (1p)
...
wordToAlpha x = let idx = fromIntegral $ mod x 26 in chr $ ord 'a' + idx
{-
    5. (1p)
...
tableToFunc t key = let subst = \(x, _) -> (x == key) in snd $ head $ filter subst t 
