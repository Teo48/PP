...
safe(X/Y/S, Others):-
    not(member(X/_/S, Others)),
    not(member(_/Y/S, Others)).
...
% dată.
parseState(stare(est, CaniEst, MisiEst), est, MisiEst, CaniEst, MisiVest, CaniVest) :-
    MisiVest is 3 - MisiEst,
    CaniVest is 3 - CaniEst.
parseState(stare(vest, CaniVest, MisiVest), vest, MisiEst, CaniEst, MisiVest, CaniVest) :-
    MisiEst is 3 - MisiVest,
    CaniEst is 3 - CaniVest.
...
% ales.
initial_state(misionari, stare(est, 3, 3)).
...
% S1.
next_state(misionari, stare(Mal1, C1, M1), stare(Mal2, C2, M2)):-
    opus(Mal1, Mal2),
    boat(M, C),
    M =< M1,
    C =< C1,
...
dfs([],[]).
dfs([H|T], [H|L]) :- \+ arc(H, _), dfs(T, L).
dfs([H|T], [H|L]) :- arc(H, Kid), append(Kid, T, X), dfs(X, L).
