...
(define (cnt n b x y)
  (cond
    [(= x y) #t]
    [(> b 10) #f]
    [(palindrome? (num->base n b)) (cnt n (add1 b) (add1 x) y)]
    [else(cnt n (add1 b) x y)]
    )
  )

(define (first-b-pal start b)
  (if (cnt (add1 start) 2 0 b)
      (add1 start)
...
(define (num-to-list n)
  (if (zero? n)
      null
   (append (num-to-list (quotient n 10)) (list (modulo n 10)))))

(define (longest-palindrome n)
   (if (palindrome? (num-to-list n))
       n
        (max (longest-palindrome (modulo n (expt 10 (sub1 (length (num-to-list n))))))
